...
type internal IBackgroundCompiler =
    ...
    abstract member CheckFileInProject:
        ...
        userOpName: string ->
            CancellableTask<FSharpCheckFileAnswer>
    ...
    abstract member CheckFileInProjectAllowingStaleCachedResults:
        ...
        options: FSharpProjectOptions *
        userOpName: string ->
            CancellableTask<FSharpCheckFileAnswer option>
    ...
    abstract member FindReferencesInFile:
        ...
        userOpName: string ->
            CancellableTask<seq<FSharp.Compiler.Text.range>>
    ...
    abstract member GetAssemblyData:
        options: FSharpProjectOptions * outputFileName: string * userOpName: string ->
            CancellableTask<FSharp.Compiler.CompilerConfig.ProjectAssemblyDataResult>
    ...
    abstract member GetBackgroundCheckResultsForFileInProject:
        fileName: string * options: FSharpProjectOptions * userOpName: string -> CancellableTask<FSharpParseFileResults * FSharpCheckFileResults>

    /// Fetch the parse information from the background compiler (which checks w.r.t. the FileSystem API)
    abstract member GetBackgroundParseResultsForFileInProject:
        fileName: string * options: FSharpProjectOptions * userOpName: string -> CancellableTask<FSharpParseFileResults>
    ...
    abstract member GetCachedCheckFileResult:
        builder: IncrementalBuilder * fileName: string * sourceText: ISourceText * options: FSharpProjectOptions ->
            CancellableTask<(FSharpParseFileResults * FSharpCheckFileResults) option>
    ...
    abstract member GetSemanticClassificationForFile:
        fileName: string * options: FSharpProjectOptions * userOpName: string ->
            CancellableTask<FSharp.Compiler.EditorServices.SemanticClassificationView option>
    ...
    abstract member NotifyFileChanged: fileName: string * options: FSharpProjectOptions * userOpName: string -> CancellableTask<unit>
    ...
    abstract member ParseAndCheckProject: options: FSharpProjectOptions * userOpName: string -> CancellableTask<FSharpCheckProjectResults>

    abstract member ParseAndCheckProject: projectSnapshot: FSharpProjectSnapshot * userOpName: string -> CancellableTask<FSharpCheckProjectResults>
...
type internal BackgroundCompiler
    ...
    let tryGetBuilder options : CancellableTask<IncrementalBuilder option * FSharpDiagnostic[]> option =
        tryGetBuilderNode options |> Option.map (fun x -> x.GetOrComputeValue())

    let tryGetSimilarBuilder options : CancellableTask<IncrementalBuilder option * FSharpDiagnostic[]> option =
        incrementalBuildersCache.TryGetSimilar(AnyCallerThread, options)
        |> Option.map (fun x -> x.GetOrComputeValue())

    let tryGetAnyBuilder options : CancellableTask<IncrementalBuilder option * FSharpDiagnostic[]> option =
    ...
    let getOrCreateBuilder (options, userOpName) : CancellableTask<IncrementalBuilder option * FSharpDiagnostic[]> =
    ...
    member private _.CheckOneFileImplAux
        ...
        ) : CancellableTask<CheckFileCacheValue> =
    ...
    member _.GetAssemblyData(options, userOpName) : CancellableTask<ProjectAssemblyDataResult> =
        cancellableTask {
            use _ =
                Activity.start
    ...
    interface IBackgroundCompiler with
        ...
        member _.GetAssemblyData
            (
                ...
                _fileName: string,
                userOpName: string
            ) : CancellableTask<ProjectAssemblyDataResult> =
        ...
        member _.GetBackgroundCheckResultsForFileInProject
            (
                ...
                options: FSharpProjectOptions,
                userOpName: string
            ) : CancellableTask<FSharpParseFileResults * FSharpCheckFileResults> =
        ...
        member _.GetBackgroundParseResultsForFileInProject
            (
                ...
                options: FSharpProjectOptions,
                userOpName: string
            ) : CancellableTask<FSharpParseFileResults> =
        ...
        member _.GetSemanticClassificationForFile
            (
                ...
                options: FSharpProjectOptions,
                userOpName: string
            ) : CancellableTask<EditorServices.SemanticClassificationView option> =
        ...
        member _.NotifyFileChanged(fileName: string, options: FSharpProjectOptions, userOpName: string) : CancellableTask<unit> =
        ...
        member _.ParseAndCheckProject(options: FSharpProjectOptions, userOpName: string) : CancellableTask<FSharpCheckProjectResults> =
            self.ParseAndCheckProject(options, userOpName)

        member _.ParseAndCheckProject(projectSnapshot: FSharpProjectSnapshot, userOpName: string) : CancellableTask<FSharpCheckProjectResults> =
        ...
        member _.ParseFile(fileName: string, projectSnapshot: FSharpProjectSnapshot, userOpName: string) =
            let options = projectSnapshot.ToOptions()
            async {
                return! self.GetBackgroundParseResultsForFileInProject(fileName, options, userOpName)
            }
