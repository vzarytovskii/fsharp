...
[<Sealed>]
type GraphNode<'T> private (computation: CancellableTask<'T>, cachedResult: ValueOption<'T>, cachedResultNode: CancellableTask<'T>) =

    let mutable computation = computation
    let mutable requestCount = 0

    let mutable cachedResult = cachedResult
    let mutable cachedResultNode: CancellableTask<'T> = cachedResultNode
                        ...
                        | _ ->
                            ...
                            Async.StartWithContinuations(
                                ...
                                (fun res ->
                                    ...
                                    cachedResultNode <- CancellableTask.singleton res
    ...
    static member FromResult(result: 'T) =
        let nodeResult = CancellableTask.singleton result
