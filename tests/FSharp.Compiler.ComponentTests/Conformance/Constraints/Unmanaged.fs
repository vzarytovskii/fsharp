...
let x = struct(1, 2)
test x
// test (struct(1, 2)) // FIXME: When we check isUnmanaged on arguments, tuple hasn't been solved yet.
        """
        |> typecheck
        |> shouldSucceed
        |> ignore
...
test(x)
//test(S<_>(struct {| X = 1; Y = 1 |})) // FIXME: When we check isUnmanaged on arguments, anon record hasn't been solved yet.
...
let _ = Test<NonStructRecdC<int>>()
        ...
        |> shouldFail
        |> withDiagnostics[
             (Error 1, Line 26, Col 6, Line 26, Col 33, "A generic construct requires that the type 'S<obj>' is an unmanaged type")
             (Error 1, Line 27, Col 6, Line 27, Col 14, "A generic construct requires that the type 'X<'a>' is an unmanaged type")
             (Error 1, Line 28, Col 7, Line 28, Col 20, "A generic construct requires that the type 'A<obj,int>' is an unmanaged type")
             (Error 1, Line 29, Col 24, Line 29, Col 36, "A generic construct requires that the type 'A<'T,obj>' is an unmanaged type")
             (Error 1, Line 31, Col 9, Line 31, Col 18, "A generic construct requires that the type 'obj' is an unmanaged type")
             (Error 1, Line 32, Col 9, Line 32, Col 28, "A generic construct requires that the type 'NonStructRecd' is an unmanaged type")
             (Error 1, Line 33, Col 9, Line 33, Col 34, "A generic construct requires that the type 'NonStructRecdC<int>' is an unmanaged type")]
    
    // let ``Modreq and attribute are emitted for unmanaged`` () =