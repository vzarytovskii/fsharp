...
type internal FrameworkImportsCache =
    ...
    member Get: TcConfig -> CancellableTask<TcGlobals * TcImports * AssemblyResolution list * UnresolvedAssemblyReference list>
...
type internal PartialCheckResults =
    ...
    member GetOrComputeTcInfo: unit -> CancellableTask<TcInfo>
    ...
    member GetOrComputeTcInfoWithExtras: unit -> CancellableTask<TcInfo * TcInfoExtras>
    ...
    member GetOrComputeItemKeyStoreIfEnabled: unit -> CancellableTask<ItemKeyStore option>

    /// Can cause a second type-check if `enablePartialTypeChecking` is true in the checker.
    /// Only use when it's absolutely necessary to get rich information on a file.
    ///
    /// Will return 'None' for enableBackgroundItemKeyStoreAndSemanticClassification=false.
    member GetOrComputeSemanticClassificationIfEnabled: unit -> CancellableTask<SemanticClassificationKeyStore option>
...
type internal IncrementalBuilder =
    ...
    member PopulatePartialCheckingResults: unit -> CancellableTask<unit>
    ...
    member GetCheckResultsBeforeFileInProject: fileName: string -> CancellableTask<PartialCheckResults>

    /// Get the preceding typecheck state of a slot. Compute the entire type check of the project up
    /// to the necessary point if the result is not available. This may be a long-running operation.
    /// This will get full type-check info for the file, meaning no partial type-checking.
    member GetFullCheckResultsBeforeFileInProject: fileName: string -> CancellableTask<PartialCheckResults>

    /// Get the typecheck state after checking a file. Compute the entire type check of the project up
    /// to the necessary point if the result is not available. This may be a long-running operation.
    member GetCheckResultsAfterFileInProject: fileName: string -> CancellableTask<PartialCheckResults>

    /// Get the typecheck state after checking a file. Compute the entire type check of the project up
    /// to the necessary point if the result is not available. This may be a long-running operation.
    /// This will get full type-check info for the file, meaning no partial type-checking.
    member GetFullCheckResultsAfterFileInProject: fileName: string -> CancellableTask<PartialCheckResults>

    /// Get the typecheck result after the end of the last file. The typecheck of the project is not 'completed'.
    /// This may be a long-running operation.
    member GetCheckResultsAfterLastFileInProject: unit -> CancellableTask<PartialCheckResults>
    ...
    member GetCheckResultsAndImplementationsForProject:
        unit ->
            CancellableTask<PartialCheckResults * IL.ILAssemblyRef * ProjectAssemblyDataResult * CheckedImplFile list option>
    ...
    member NotifyFileChanged: fileName: string * timeStamp: DateTime -> CancellableTask<unit>
    ...
    static member TryCreateIncrementalBuilderForProjectOptions:
        ...
        useSyntaxTreeCache: bool ->
            CancellableTask<IncrementalBuilder option * FSharpDiagnostic[]>
