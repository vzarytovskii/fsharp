...
and [<StructuralEquality; NoComparison>] ExprConstInfo =
    | AppOp
    | IfThenElseOp
    | LetRecOp
    | LetRecCombOp
...
and [<CompiledName("FSharpExpr`1")>] Expr<'T>(term: Tree, attribs) =
...
module Patterns =
    ...
    let u_NamedType st =
        ...
        if System.Int32.TryParse(a, &idx) && b = "" then
            ...
            let escapedTcName =
                System.Text.RegularExpressions.Regex.Replace(a, @"(?<!\\), ", @"\, ")
...
module DerivedPatterns =
    ...
    let (|Unit|_|) input =
        match input with
        | Comb0 (ValueOp (_, ty, None)) when ty = typeof<unit> -> Some()
        | _ -> None
